...
func getRequestBodyFromEvent(event *Event) []byte {
	...
	body, err = json.Marshal(event)
	if err == nil {
		Logger.Println("Original event couldn't be marshalled. Succeeded by stripping contextual data. " +
			"Please verify that the data you attach to the scope is serializable.")
		return body
	}
