...
func TestFixedRateSampler(t *testing.T) {
	...
	t.Run("Inheritance", func(t *testing.T) {
		...
		for want, parentDecision := range [...]Sampled{SampledUndefined, SampledFalse, SampledTrue} {
			rootSpan.Sampled = parentDecision
	...
	t.Run("UniformRate", func(t *testing.T) {
		...
		for _, tt := range tests {
			...
			t.Run(fmt.Sprint(tt.Rate), func(t *testing.T) {
				s.Rate = tt.Rate
				got := repeatedSample(s, SamplingContext{Span: rootSpan}, 10000)
